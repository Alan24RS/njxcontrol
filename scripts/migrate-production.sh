...
if [ "$IS_LOCAL" = true ]; then
    ...
    echo ""
    echo "🌱 Seeding datos de reportes (recaudación por playa)..."
    # Validar clave de servicio necesaria para escritura con bypass de RLS
    if [ -z "$SUPABASE_SERVICE_ROLE_KEY" ]; then
        echo "❌ Error: SUPABASE_SERVICE_ROLE_KEY no está definido."
        echo "   Debe configurarse en variables de entorno del despliegue (CI) o en .env.local."
        exit 1
    fi
    # En CI ya tenemos variables de entorno cargadas; en local usamos .env.local
    if [ "$CI" = "true" ] || [ "$GITHUB_ACTIONS" = "true" ] || [ "$VERCEL" = "1" ]; then
        echo "   Ejecutando seed con entorno de CI"
        node --import tsx scripts/seed-recaudacion-reportes.ts
    else
        echo "   Ejecutando seed con .env.local"
