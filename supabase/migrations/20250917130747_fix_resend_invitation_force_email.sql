-- =====================================================
-- MIGRACIÓN: CORREGIR REENVÍO DE INVITACIÓN PARA FORZAR EMAIL
-- =====================================================
-- Modifica la función reenviar_invitacion_playero para que siempre indique que requiere email
...
BEGIN
    ...
    -- Buscar invitación pendiente (incluso si está expirada)
    ...
    -- RESETEAR COMPLETAMENTE: Marcar invitación anterior como expirada
    UPDATE public.playero_invitacion 
    SET estado = 'EXPIRADA',
        fecha_modificacion = now()
    WHERE invitacion_id = v_invitacion_id;
    
    -- Crear una nueva invitación con datos frescos (SIN fecha_creacion)
...
COMMENT ON FUNCTION public.reenviar_invitacion_playero(text, uuid) IS 'Reenvía una invitación creando una nueva entrada y marcando la anterior como expirada. Siempre requiere reenvío de email.';
