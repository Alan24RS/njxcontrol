...
BEGIN
  ...
  IF v_dia_actual != v_ultimo_dia_mes THEN
    RETURN jsonb_build_object(
      ...
      'boletas_generadas', 0,
      'errores', '[]'::jsonb,
      'mensaje', 'No es el último día del mes. No se generan boletas.',
  ...
  RETURN jsonb_build_object(
    ...
    'dia_actual', v_dia_actual,
    'ultimo_dia_mes', v_ultimo_dia_mes,
    'mensaje', 'Boletas generadas exitosamente en el último día del mes'
  );
END;
$$;

COMMENT ON FUNCTION public.generar_boletas_mensuales IS 
  'Genera boletas mensuales para todos los abonos activos ÚNICAMENTE el último día del mes. Las boletas tienen 15 días corridos de vencimiento. Se ejecuta vía cron job.';
