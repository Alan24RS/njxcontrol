...
BEGIN
  ...
  BEGIN
    ...
    RETURNING numero_de_boleta INTO v_numero_boleta;

    PERFORM pg_advisory_xact_lock(hashtext('pago_' || p_playa_id::text));
...
COMMENT ON FUNCTION public.create_abonado_with_abono IS 
  'Crea o actualiza un abonado y genera un abono con vehículos, boleta inicial pagada y registro de pago. Vincula todo al turno activo del playero.';
