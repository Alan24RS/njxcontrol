...
BEGIN
  IF NOT EXISTS (
    ...
    ADD COLUMN turno_creacion_playa_id UUID NOT NULL;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.abono 
    ADD COLUMN turno_creacion_playero_id UUID NOT NULL;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.abono 
    ADD COLUMN turno_creacion_fecha_hora_ingreso TIMESTAMPTZ NOT NULL;
  END IF;
END $$;
...
COMMENT ON COLUMN public.abono.turno_creacion_playa_id IS 'ID de la playa del turno en que se creó el abono';
COMMENT ON COLUMN public.abono.turno_creacion_playero_id IS 'ID del playero que creó el abono';
COMMENT ON COLUMN public.abono.turno_creacion_fecha_hora_ingreso IS 'Fecha/hora de ingreso del turno en que se creó el abono';
COMMENT ON COLUMN public.abono.turno_finalizacion_playa_id IS 'ID de la playa del turno en que se finalizó el abono (NULL si aún está activo)';
COMMENT ON COLUMN public.abono.turno_finalizacion_playero_id IS 'ID del playero que finalizó el abono (NULL si aún está activo)';
COMMENT ON COLUMN public.abono.turno_finalizacion_fecha_hora_ingreso IS 'Fecha/hora de ingreso del turno en que se finalizó el abono (NULL si aún está activo)';
