-- Corregir la función para actualizar un tipo de plaza con sus características
...
BEGIN
  ...
  IF array_length(p_caracteristicas, 1) > 0 THEN
    ...
    FOR v_existing_caracteristicas IN 
      ...
      FROM tipo_plaza tp 
      LEFT JOIN tipo_plaza_caracteristica tpc ON tp.tipo_plaza_id = tpc.tipo_plaza_id
      WHERE tp.playa_id = v_playa_id 
        AND tp.tipo_plaza_id != p_tipo_plaza_id
        AND tp.fecha_eliminacion IS NULL
