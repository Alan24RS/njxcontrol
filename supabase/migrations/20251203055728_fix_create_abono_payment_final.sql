-- Restaurar comportamiento: si se proporciona método de pago y monto en create_abonado_with_abono,
-- se debe registrar el pago automáticamente y marcar la boleta como PAGADA.
-- Esto corrige el flujo del wizard donde el usuario paga en el paso 3.
  ...
  BEGIN
    ...
    IF p_metodo_pago IS NOT NULL AND p_monto_pago IS NOT NULL AND p_monto_pago > 0 THEN
      ...
      INSERT INTO public.pago (
        ...
        p_playa_id,
        v_numero_pago,
        v_boleta_id, -- Vincular con la boleta creada
        NOW(),
        p_monto_pago,
        p_metodo_pago,
        p_turno_playero_id,
...
COMMENT ON FUNCTION public.create_abonado_with_abono IS 
  'Crea o actualiza un abonado y genera un nuevo abono con su boleta inicial. Si se proporcionan p_metodo_pago y p_monto_pago, registra el pago automáticamente y marca la boleta como PAGADA. Si no, la boleta queda PENDIENTE.';
