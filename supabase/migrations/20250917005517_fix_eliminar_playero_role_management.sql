-- =====================================================
-- MIGRACIÓN: CORREGIR GESTIÓN DE ROLES AL ELIMINAR PLAYERO
-- =====================================================
-- Mejora la función eliminar_playero para gestionar correctamente los roles

-- Función mejorada para eliminar playero con gestión de roles
...
BEGIN
    ...
    -- Contar otras relaciones playero_playa activas del usuario (excluyendo la actual)
    ...
    ELSE
        ...
        ELSE
            v_result := json_build_object(
                ...
                'action', 'deleted',
                'role_removed', false,
                'message', format('Playero %s eliminado de la playa (mantiene rol PLAYERO por otras asignaciones)', v_playero_nombre)
            );
        END IF;
    END IF;
...
COMMENT ON FUNCTION public.eliminar_playero(uuid, uuid, text) IS 'Elimina o suspende un playero según si tiene referencias en otras tablas. Gestiona automáticamente el rol PLAYERO según las relaciones restantes.';
