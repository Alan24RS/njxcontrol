...
/**
 * Get daily timeline data for a specific playero's performance
 * Shows: hours worked per day, closed occupations, revenue generated
...
export const getPerformancePlayeroTimeline = cache(
  async (
    ...
    if (!args.playero_id) {
      return {
        data: null,
        error: 'ID de playero es requerido'
      }
    }
    ...
    try {
      ...
      const { data, error } = await supabase.rpc(
        'analytics_performance_playero_timeline',
        {
          ...
          p_intervalo: args.intervalo || 'diario'
        }
      )
